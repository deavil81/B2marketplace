...
class HomeController extends Controller
{
    ...
    private function suggestProductsBasedOnActivity($userId)
    {
        ...
        // Fetch products based on viewed products or explored categories
        $query = Product::query();

        if ($exploredCategoryIds->isNotEmpty()) {
            $query->whereIn('category_id', $exploredCategoryIds);
        }

        if ($viewedProductIds->isNotEmpty()) {
            $query->orWhereIn('id', $viewedProductIds);
        }

        $suggestedProducts = $query->distinct()->take(8)->get();

        // If no suggested products found, return random products
        if ($suggestedProducts->isEmpty()) {
            \Log::info('No suggested products found, returning random products');
            return Product::inRandomOrder()->take(8)->get();
        }
    ...
    { 
        $unreadNotifications = auth()->check() 
            ...
            : 0; 
        $messages = auth()->check() 
            ? Message::where('receiver_id', Auth::id())->with('sender')->get() 
            : collect(); 
